...
A.k.a summary functions

- Starndard statistics: `max()`, `min()`, `median()`, `mean()`, etc...
...
white_wine |> 
  select(alcohol) |>
  mutate(a_m_median = alcohol - median(alcohol), .keep = "none")

white_wine |> 
  summarise(mean(alcohol),sd(alcohol),median(alcohol),IQR(alcohol))

white_wine <- white_wine |> mutate(id = row_number())
white_wine2 <- white_wine |> select(id, alcohol) |> rename(alcohol2 = alcohol)

white_wine3 <- left_join(white_wine,white_wine2)
...
```{r}
white_wine |> summarise(quantile(`residual sugar`))

white_wine |> reframe(quantile(`residual sugar`), mean(`residual sugar`))
```
```{r}
...
```{r}
white_wine |>
  mutate(class = cut(alcohol, breaks = c(7,9,10,12,13,15))) |>
  summarise(median(`residual sugar`),
                   n(),
                   .by = class)
```
...
```
```{r}
white_wine |> pull(alcohol) |> quantile(probs = seq(0,1, by = 0.2))
```
```{r}
alcohol_quantiles <- 
  white_wine |> 
...
#I will modify the first element
...
white_wine |> 
  ...
  select(1:2 | quality) # first 2 columns
  
```

```{r}
white_wine |> 
  slice_sample(n=1, by = quality) |> ##first 5 rows
  select(1:2 | quality) # first 2 columns
```
```{r}
white_wine |> 
  slice_max(alcohol, by = quality) |> ##first 5 rows
  select(1:2 | quality) # first 2 columns
```
```{r}
...
world_bank_pop |> 
  ...
  filter(country %in% c("FRA","ESP","DEU")) |>
...
```{r}
set.seed(42) #select the start of the randomness and makes sure that the same random numbers are produced all the time
world_bank_pop |> 
  pivot_longer(cols = !country & !indicator , names_to = "year", values_to = "value") |>
  filter(country %in% c("FRA","ESP","DEU")) |>
  ggplot(aes(x=as.numeric(year),y=value,color = country)) +
